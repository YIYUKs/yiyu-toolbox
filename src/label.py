...
class LaMa:
    ...
    def __init__(self, crop_trigger_size: List[int], crop_margin: int, device):
        ...
        self.device = device
        model = torch.jit.load(".\\model\\big-lama.pt", map_location="cpu")
...
class WorkThread(QThread):
    ...
    def process(self):
        ...
        mask = norm_img(mask)
        print(f"Resized image shape: {mask.shape}")
        start = time.time()
...
class Label(QtWidgets.QWidget):
    ...
    def timeStop(self, str):
        if str == "ok":
            ...
            # Re-enable parent buttons (Need way to access parent controls or signals)
            # The original code accessed self.btn_org_2 via direct attribute assignment from main window
            if hasattr(self, 'btn_org_2'):
