...
:: 使用 python -m PyInstaller 调用
python -m PyInstaller --noconfirm --onedir --windowed --clean ^
    --name "乙羽的工具箱" ^
    --icon "%CORE_DIR%/resources/icon.ico" ^
    --add-data "%CORE_DIR%/resources;resources" ^
    --add-data "%CORE_DIR%/model;model" ^
    --add-binary "%CORE_DIR%/msvcp140.dll;." ^
    --add-binary "%CORE_DIR%/msvcp140_1.dll;." ^
    --add-binary "%CORE_DIR%/msvcp140_2.dll;." ^
    --add-binary "%CORE_DIR%/vcruntime140.dll;." ^
    --add-binary "%CORE_DIR%/vcruntime140_1.dll;." ^
    --collect-all "torch" ^
