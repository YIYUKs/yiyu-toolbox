    ...
    def run(self):
        try:
            ...
            for idx, filename in enumerate(image_files):
                ...
                self.progress_signal.emit(0, total_prog, f"[{idx+1}/{total}] 处理中: {filename}")
                
                # Read Image
                image = cv2.imdecode(np.fromfile(input_path, dtype=np.uint8), -1)
                if image is None: continue
                
                self.progress_signal.emit(30, total_prog, f"[{idx+1}/{total}] AI 推理中...")
                ...
                img_norm = norm_img(image) # Returns torch tensor or numpy? norm_img in helper says... let's check helper
            ...
            self.progress_signal.emit(100, 100, "批量处理完成！")
